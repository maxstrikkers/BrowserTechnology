...
document.addEventListener('DOMContentLoaded', function() {
  ...
  herstelFormulier(formulier); // Functie die de opgeslagen waarden herstelt
  // localStorage.clear(); // Verwijderd alle opgeslagen waarden
//   // Opslaan van wijzigingen
  formulier.querySelectorAll('input, select, textarea').forEach(element => {
      element.addEventListener('change', function() {
          if (element.type === 'radio') {
              // Voor radio buttons: sla alleen de waarde op als deze geselecteerd is
              if (element.checked) {
...
function herstelFormulier(formulier) {
  formulier.querySelectorAll('input, select, textarea').forEach(element => {
      if (localStorage.getItem(element.name)) {
          const opgeslagenWaarde = localStorage.getItem(element.name);
          if (element.type === 'radio') {
              // Voor radio buttons: markeer als 'checked' als de opgeslagen waarde overeenkomt
              if (element.value === opgeslagenWaarde) {
                  element.checked = true;
              }
          } else {
              // Voor alle andere velden
              element.value = opgeslagenWaarde;
          }
      }
